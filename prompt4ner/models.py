...
class Prompt4NER(PreTrainedModel):
    ...
    def __init__(
        ...
        self.query_embed = nn.Embedding(prompt_number, config.hidden_size * 2).to('cuda:0')
        ...
        if self.decoder_layers > 0:
            ...
            self.decoder = DetrTransformerDecoder(decoder_layer=decoder_layer, num_layers=self.decoder_layers).to('cuda:0')
            if self.withimage:
                self.img_decoder = DetrTransformerDecoder(decoder_layer=decoder_layer, num_layers=self.decoder_layers).to('cuda:0')
            if self.prompt_length>1:
                self.decoder2 = DetrTransformerDecoder(decoder_layer=decoder_layer, num_layers=self.decoder_layers).to('cuda:0')
        ...
        if model_type == "bert":
            ...
            self.bert = BertModel(config, prompt_ids = prompt_token_ids).to('cuda:1')
            ...
            self.cls = None
            self.entity_classifier = EntityTypePredictor(config, entity_type_count, lambda x: self.cls(x)).to('cuda:0')
        ...
        if self.withimage:
            self.vision2text = nn.Linear(clip_v.config.hidden_size, config.hidden_size).to('cuda:0')
        ...
        if self.lstm_layers > 0:
            self.lstm = nn.LSTM(input_size = config.hidden_size, hidden_size = config.hidden_size//2, num_layers = lstm_layers,  bidirectional = True, dropout = 0.1, batch_first = True).to('cuda:0')
        ...
        self.left_boundary_classfier = EntityBoundaryPredictorBak(config, self.prop_drop).to('cuda:0')
        self.right_boundary_classfier = EntityBoundaryPredictorBak(config, self.prop_drop).to('cuda:0')
    ...
    def _common_forward(
        ...
        context_masks = context_masks.float().to('cuda:0')
        token_masks = token_masks.to('cuda:0')
        ...
        outputs = model(
                    ...
                    attention_mask=context_masks.to('cuda:1'),
        ...
        if self.use_masked_lm and self.training:
            if self.model_type == "bert":
                masked_seq_logits = self.cls(outputs.last_hidden_state.to('cuda:0'))
            if self.model_type == "roberta":
                masked_seq_logits = self.lm_head(outputs.last_hidden_state.to('cuda:0'))
        ...
        for i in range(self.loss_layers, 0, -1):
            ...
            h = outputs.hidden_states[-1].to('cuda:0')
            h_token = util.combine(h, context2token_masks.to('cuda:0'), self.pool_type).to('cuda:0')
            ...
            if inx4locator is not None:
                ...
                tgt = util.batch_index(outputs.hidden_states[-i].to('cuda:0'), inx4locator.to('cuda:0')) + orig_tgt
                if self.prompt_length > 1:
                    tgt2 = util.batch_index(outputs.hidden_states[-i].to('cuda:0'), inx4locator.to('cuda:0') + self.prompt_length-1) + orig_tgt
