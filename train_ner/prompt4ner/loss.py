...
class Prompt4NERLoss(Loss):
    ...
    def compute(self, entity_logits, pred_left, pred_right, output, gt_types, gt_spans, entity_masks, epoch, deeply_weight = "same", seq_logits = None, gt_seq_labels = None, batch = None):
        ...
        targets = {"labels": gt_types_wo_nil.to('cuda:0'), "gt_left":spans_wo_nil[:, 0].to('cuda:0'), "gt_right":spans_wo_nil[:, 1].to('cuda:0'), "sizes":sizes}

        train_loss = []
        indices = None
        for out_dict in output[::-1]:
            entity_logits, pred_left, pred_right = out_dict["entity_logits"].to('cuda:0'), out_dict["p_left"].to('cuda:0'), out_dict["p_right"].to('cuda:0')
...
class Criterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_spans):
        ...
        if self.type_loss == "celoss":
            ...
            loss_ce = F.cross_entropy(src_logits.to('cuda:0'), target_classes.to('cuda:0'), empty_weight.to('cuda:0'), reduction='none')
    ...
    def loss_boundary(self, outputs, targets, indices, num_spans):
        ...
        token_masks = token_masks[idx].to('cuda:0')
        ...
        left_nll_loss = F.binary_cross_entropy(src_spans_left, left_onehot, reduction='none').to('cuda:0')
        right_nll_loss = F.binary_cross_entropy(src_spans_right, right_onehot, reduction='none').to('cuda:0')
